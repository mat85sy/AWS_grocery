...
6.  **Start the Application Manually**:
    ...
    ```
    d. Install Dependencies (Optional - Dependencies are installed automatically during instance creation, but run this to ensure they are up-to-date)
...
1.  **Verify Docker Installation and Navigate:**
    ...
    ```
 2. **Populate Database:**:
    a.Use the `psql` command-line tool to load the initial data from the provided SQL dump file into your RDS instance. Replace `<rds-endpoint>` with the actual endpoint of           your RDS instance (found in the AWS console or Terraform outputs).
       ```bash
...
3.  **Configure Environment Variables:**
    ...
    echo "POSTGRES_PASSWORD=YourSecretPassword123" >> .env
    echo "POSTGRES_DB=grocerymate_db" >> .env
    echo "POSTGRES_HOST=<rds-endpoint>" >> .env
    echo "POSTGRES_URI=postgresql://grocery_user:YourSecretPassword123@<rds-endpoint>:5432/grocerymate_db" >> .env
    ```
...
*   Avatars are uploaded to the configured S3 bucke
